...
void UsingPath() 
{
	fs::path selectedPath{R"(./main.cpp)"} ;

	std::cout << selectedPath << std::endl;

	// Prints without 2 backslashes
	std::cout << selectedPath.string() << std::endl;

	// Removes filename part
	selectedPath.remove_filename() ;
	
	// Adds filename at end
	selectedPath /= "main.cpp" ;
...
int filesys() 
{
	std::cout << "** filesystem c++17 **" << std::endl;

	UsingPath();
	TraversingDirectory(R"(./)") ;
	//DirectoryOperations(R"(E:\Data)") ;
	//Permissions(R"(/var/tmp/test.dat)") ;
	return(0);
}
